...
class CNN(nn.Module): #Hereda de nn.Module que contiene modulos de PyTorch para la construccion de modelos de RNAs
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64 * 3 * 3, 128) #Capa que recibe 32 mapas de caracteristicas de 7x7 generando 128 neuronas de salida
    ...
    def forward(self, tensor): #Funcion para definir el flujo de datos
        tensor = self.pool(F.selu(self.conv1(tensor))) #Pasa el tensor a traves de la primera capa convolucional generando 16 mapas de caracteristicas. Usa la funcion de activacion ReLU (Rectified Linear Unit). Al pasar el resultado por la capa pool se reducen las dimensiones de cada mapa
        tensor = self.pool(F.selu(self.conv2(tensor))) #Pasa el resultado de la anterior por la segunda capa convolusional generando 32 mapas de caracteristicas. Usa ReLU y reduce el tamano de los mapas con la capa pool
        tensor = self.pool(F.selu(self.conv3(tensor)))
        tensor = tensor.view(-1, 64 * 3 * 3) #Convierte los mapas en vectores lineales
        tensor = F.selu(self.fc1(tensor)) #Pasa los resultados por la primera capa totalmente conectada activada por ReLU
